...
const SnowflakeIcon = () => (
  <svg viewBox="0 0 24 24" fill="none" stroke="#38bdf8" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" className="w-7 h-7">
    ...
    <polyline points="17 7 12 2 7 7" /><polyline points="7 17 12 22 17 17" />
    <polyline points="2 7 7 12 2 17" /><polyline points="22 17 17 12 22 7" />
  </svg>
);

const PaletteIcon = () => (
  <svg viewBox="0 0 24 24" fill="none" stroke="#a855f7" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" className="w-7 h-7">
    <circle cx="13.5" cy="6.5" r="0.5" fill="#a855f7" /><circle cx="17.5" cy="10.5" r="0.5" fill="#a855f7" />
    <circle cx="8.5" cy="7.5" r="0.5" fill="#a855f7" /><circle cx="6.5" cy="12.5" r="0.5" fill="#a855f7" />
...
const LeafIcon = () => (
  <svg viewBox="0 0 24 24" fill="none" stroke="#22c55e" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" className="w-7 h-7">
    <path d="M17 8C8 10 5.9 16.17 3.82 19.34" /><path d="M2 2s3 1 7 6c4 5 4 10 4 10S10 10 2 2z" />
  </svg>
);
        ...
        {steps.map((label, i) => {
          ...
          return (
            <div key={step} className="flex flex-col items-center gap-1">
              ...
              <span className={`text-xs ${isActive || isDone ? "text-gray-700 font-medium" : "text-gray-400"}`}>{label}</span>
            </div>
          );
        })}
      </div>
...
const Step2 = ({ onNext }: { onNext: () => void }) => {
  ...
  return (
    <div className="flex flex-col flex-1 px-5 py-6 overflow-auto">
      ...
      <div className="mb-2">
        <h3 className="text-lg font-bold text-gray-900 mb-1">Verify Your Identity</h3>
        <p className="text-sm text-gray-400 mb-4">Upload required documents to continue</p>
        ...
        <div className="border border-gray-100 rounded-2xl overflow-hidden divide-y divide-gray-100 bg-white shadow-sm">
          {docs.map((doc) => (
            <div key={doc.key} className="flex items-center gap-3 px-4 py-4">
              ...
              >
                <UploadIcon />
                {uploaded[doc.key] ? <span className="text-green-500">✓</span> : "Choose"}
              </button>
            </div>
          ))}
        </div>
...
const Step3 = ({ onNext }: { onNext: () => void }) => {
  ...
  return (
    <div className="flex flex-col flex-1 px-5 py-6 overflow-auto">
      <h3 className="text-xl font-bold text-gray-900 mb-1">Set Up your services</h3>
      <p className="text-sm text-gray-400 mb-5">Tell us what service You offer</p>

      <h4 className="text-base font-bold text-gray-800 mb-3">Select categories</h4>
      ...
      <div className="border border-gray-100 rounded-2xl p-4 bg-white shadow-sm mb-6">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            <p className="text-sm font-bold text-gray-800">Radius: <span className="text-amber-500">{radius} Km</span></p>
          ...
          {items.map((item, i) => (
            <div key={i} className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                {item.done
                  ? <svg className="w-4 h-4 text-amber-400" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12" /></svg>
                  : <ClockIcon />
                }
                <span className={`text-sm ${item.badge ? "text-amber-500 font-medium" : "text-gray-700"}`}>{item.label}</span>
              </div>
              {item.badge
                ? <span className="text-xs bg-amber-50 text-amber-500 border border-amber-200 px-2 py-0.5 rounded-full font-medium">{item.badge}</span>
                : <svg className="w-4 h-4 text-amber-400" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12" /></svg>
              }
            </div>
          ))}
...
const Step5 = ({ onStart }: { onStart: () => void }) => {
        ...
        <div className="px-6 pb-2">
          ...
          <div className="flex gap-4">
            <div className="flex flex-col items-center">
              <div className="w-11 h-11 rounded-full bg-amber-400 flex items-center justify-center flex-shrink-0 shadow-sm">
                <LockIcon />
              </div>
              {/* Connector */}
            ...
            <div className="pt-1 pb-4">
              ...
              <p className="text-xs text-gray-400 leading-relaxed mt-0.5">Get full access and see your mindset start to<br />change.</p>
            </div>
          </div>
          ...
          <div className="flex gap-4">
            <div className="flex flex-col items-center">
              ...
              {/* Connector */}
              <div className="w-0.5 my-1" style={{ height: 36, background: "linear-gradient(to bottom, #fbbf24 0%, #6b7280 100%)" }} />
            </div>
            <div className="pt-1 pb-4">
              <p className="text-sm font-bold text-gray-900">Day 2</p>
              <p className="text-xs text-gray-400 leading-relaxed mt-0.5">Get a reminder that your trial ends in 24<br />hours</p>
            </div>
          </div>
          ...
          <div className="flex gap-4">
            <div className="flex flex-col items-center">
              <div className="w-11 h-11 rounded-full bg-gray-800 flex items-center justify-center flex-shrink-0 shadow-sm">
                <CrownIcon />
              </div>
            </div>
            <div className="pt-1 pb-2">
              <p className="text-sm font-bold text-gray-900">After day 3</p>
              <p className="text-xs text-gray-400 leading-relaxed mt-0.5">Your free trial ends and you'll be charged,<br />cancel anytime before</p>
            </div>
          </div>
        </div>
...
export default function OnboardingFlow() {
  ...
  if (showDashboard) {
    return <Dashboard onReset={() => { setStep(1); setShowDashboard(false); }} />;
  }

  return (
    <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
      <div className="w-full max-w-sm bg-white rounded-3xl shadow-xl overflow-hidden flex flex-col min-h-[700px]">
        {step !== 5 && <StepIndicator currentStep={step} />}
        {step === 1 && <Step1 onNext={() => setStep(2)} />}
        {step === 2 && <Step2 onNext={() => setStep(3)} />}
        {step === 3 && <Step3 onNext={() => setStep(4)} />}
        {step === 4 && <Step4 onNext={() => setStep(5)} />}
        {step === 5 && <Step5 onStart={() => {}} />}
      </div>
    </div>
  );
}