...
// This ensures the page is treated as dynamic in production
export const dynamic = "force-dynamic";
...
export default function ServiceDetails({ params }: Props) {
  // Unwrap params using React 'use' hook for Next.js 15 Client Components
  ...
  // Sync the URL category with the Form Select box
  useEffect(() => {
    if (category) {
  ...
  const handleSendRequest = () => {
    ...
    setTimeout(() => {
      setIsModalOpen(false);
      // Constructing the nested route we discussed earlier
      router.push(`/user-dashboard/${category}/category`);
    }, 2800);
  };
  ...
  return (
    <>
      <div className="w-full mx-auto grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            <Form form={form} layout="vertical" requiredMark={false}>
              ...
              >
                <Select
                  ...
                  className="w-full h-12 rounded-xl"
                  disabled // Disabled because it's determined by the URL
                />
              </Form.Item>
        ...
        <div className="lg:col-span-4">
          <div className="bg-white p-8 rounded-3xl shadow-xl shadow-gray-100/50 border border-gray-50 sticky top-8">
            <h2 className="text-xl font-bold mb-8 text-gray-900">
              Request Summary
            </h2>
            ...
            <Button
              block
              onClick={handleSendRequest}
              className="h-16 bg-[#F59E0B] hover:bg-[#D97706]! text-white border-none font-bold text-lg rounded-2xl shadow-xl shadow-orange-200 flex items-center justify-center gap-2"
            >
              Send Request <LuArrowRight />
            </Button>
          </div>
      ...
      >
        <div className="space-y-10">
          <div className="flex items-center gap-5">
            <div className="w-16 h-16 bg-[#FFF7E8] text-[#F59E0B] rounded-2xl flex items-center justify-center">
              <LuSparkles size={32} className="animate-pulse" />
            </div>
            <div>
              <h2 className="text-3xl font-black text-gray-900 tracking-tight">
                Analyzing Issue
          ...
          <div className="bg-[#F8FAFC] rounded-3xl p-8 space-y-6 border border-gray-100">
            {steps.map((step) => (
              <div key={step.id} className="flex items-center gap-5">
                <div
                  className={`w-10 h-10 rounded-full flex items-center justify-center transition-all ${loadingStep >= step.id ? "bg-[#F59E0B] text-white shadow-lg shadow-orange-100" : "bg-white text-gray-300 border border-gray-100"}`}
                >
                  {loadingStep > step.id ? (
                    <LuUserCheck size={20} />
                  ) : (
                ...
                <p
                  className={`font-black text-sm tracking-wide ${loadingStep >= step.id ? "text-gray-800" : "text-gray-300"}`}
                >
                  {step.label}
                </p>
              </div>
